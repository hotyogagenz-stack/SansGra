...
function renderEnhancedSuggestions(suggestions, dropdown, dropdownId, query) {
    ...
    suggestions.forEach((item, index) => {
        ...
        if (item.key && item.label) {
            // Dhatu format
            devanagari = item.key;
            hinglish = devanagariToRomanAscii ? devanagariToRomanAscii(item.key) : item.key;
            meaning = item.label;
            type = 'धातु';
            ...
            // Upasarga format
            devanagari = item.id;
            hinglish = devanagariToRomanAscii ? devanagariToRomanAscii(item.id) : item.id;
            meaning = item.label || '';
            type = 'उपसर्ग';
        ...
        // Use clean value (without HTML tags) for data-value and onclick
        const cleanValue = item.key || item.id || '';
        ...
        const inputId = inputMap[dropdownId] || dropdownId;
        ...
        html += `
            <div class="suggestion-item" 
                 ...
                 data-index="${index}" 
                 data-score="${score}"
                 onclick="selectSuggestion('${inputId}', '${escapedValue}')">
                <div class="suggestion-main">
                    <div class="suggestion-devanagari">${devanagari}</div>
                    <div class="suggestion-details">
                        <div class="suggestion-hinglish">
                            ${hinglish ? `<span class="suggestion-roman">${hinglish}</span>` : ''}
...
function getEnhancedSuggestions(query, type) {
    ...
    if (typeof sanskritDatabase === 'undefined' || !sanskritDatabase) {
        console.warn('sanskritDatabase not loaded yet');
        return results;
    }
    ...
    if (type === 'upasarga') {
        ...
        const pratyayas = (typeof pratyayaDB !== 'undefined' && pratyayaDB) ? pratyayaDB : {};
